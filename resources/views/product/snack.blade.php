            ...
            <div class="row">
                @foreach ($snacks as $snack)
                    <div class="card align-items-center" style="width: 18rem; border-radius:30px;margin: 0 30px">
                        <img src="{{ asset('storage/images/' . $snack->pphoto) }}" class="rounded-circle mx-3 my-3" alt="..." width="80%">
                        <div class="card-body text-center">
                        <h3 class="card-title">{{ $snack->pname }}</h3>
                        <h4 class="card-text">Rp.{{ $snack->pprice }}</h4>
                        <p class="btn-holder my-3"><a href="{{ route('add_to_cart', $snack->id) }}" class="btn btn-primary btn-block text-center" role="button">Add to cart</a> </p>
                        </div>
                    </div>
                @endforeach
            </div>
